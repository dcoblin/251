...
```{r}
#table_1.2df <- euro_mpx_cases %>% select( EU_region, "0-1000", "1000-9999", 
#                                          "10000-99999", "100000-199999", 
#                                          "200000-499999" , "500000-999999", 
#                                          "GE1000000") 
#table_1.2df1<- pivot_longer(table_1.2df, cols = 3:9, names_to = "city_pop", 
#                            values_to = "counts" )

#I believe the issue is with the formation of the information below table_1.2df2 <- table_1.2df1 %>% group_by(EU_region, city_pop) %>% summarize(count = sum(counts))

#table_1.2 <- table_1.2df1 %>% tabyl(city_pop, EU_region)
...
### Bar Graph
In the bar graph (fig. 1) above we see the breakdown of monthly Monkeypox infections in Europe between May and August of this year. The majority of cases occured in Western Europe and Southern Europe as evidenced by the respectively colored purple and green regions in the bar graph. Over the course of 4 months, the total number of Monkeypox cases by region were Western Asia = 4, Eastern Europe = 277, Northern Europe = 619, Southern Europe = 7958, and Western Europe = 8214
...
### Line Plot
As a follow up to the bar graph (fig. 1) above, we wanted to look at the rate of Monkeypox infections per region. In the bar graph above, Western Europe had the greatest number of Monkeypox infections however when we look at the rate of infections (fig. 2), we see that over the course of the 4 months, Southern Europe had the greatest rate of infection with a peak of 2.84 monkeypox cases per 100,000 people in the month of July.
<<<<<<< HEAD
...
```{r}
euro_census_stats3 <- read_csv("files/euro_census_stats.csv")
euro_census_stats3 <- euro_census_stats3 %>%
  mutate(AGE = case_when(AGE %in% c("Y_LT15") ~ "Age < 15",
                                  AGE %in% c("Y15-29") ~ "Age 15-29",
                                  AGE %in% c("Y30-49") ~ "Age 30-49",
                                  AGE %in% c("Y50-64") ~ "Age 50-64",
...
demo_data2 <- left_join(euro_census_stats3, euro_mpx_cases2, by = "COUNTRY_CODE") %>%
  drop_na(total_cases) %>%
   mutate(COUNTRY_CODE = case_when(COUNTRY_CODE %in% c("AT") ~ "Austria",
                                   COUNTRY_CODE %in% c("BE") ~ "Belgium",
                                   COUNTRY_CODE %in% c("BG") ~ "Bulgaria",
  ...
  summarise(sex_cat_pct = sum(age_cat_pct))

demo_data2 %>% 
  ggplot(., aes(x = COUNTRY_CODE, y = sex_cat_pct)) +
  geom_bar(aes(fill = SEX), stat = "identity") +
  theme_minimal() +
  labs(title = "Sex Distribution Among Countries With Confirmed Monkeypox Cases",
       subtitle = "Countries with at least 100 confirmed cases",
       tag = "Figure 4",
       x = "Country", 
       y = "Percentage") +
...
### Stacked Bargraph Sex Distribution
