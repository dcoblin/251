...
#I believe the issue is with the formation of the information below
table_1.2df2 <- table_1.2df1 %>% 
  group_by(EU_region, city_pop) %>% 
  summarize(count = sum(counts))


table_1.2 <- table_1.2df1 %>%
  tabyl(city_pop, EU_region)
...
euro_mpx_cases %>% 
  ...
  theme_minimal() +
  labs(title = "Figure 1: Monthly Cases of Monkeypox in Europe", x = "Month (2022)", 
       y = "Monkeypox Cases") +
...
### Figure 1
In the bar graph above we see the breakdown of monthly Monkeypox infections in Europe between May and August of this year. The majority of cases occurred in Western Europe and Southern Europe as evidenced by the respectively colored purple and green regions in the bar graph. Over the course of 4 months, the total number of Monkeypox cases by region were Western Asia = 4, Eastern Europe = 277, Northern Europe = 619, Southern Europe = 7958, and Western Europe = 8214
...
ggplot(data=graph1, aes(x=floor_month, y=monthly_rate_region, group= EU_region)) +
  ...
  theme_minimal() +
  labs(title = "Figure 2: Rate of Monkeypox Infections", x = "Month (2022)", 
...
### Figure 2
Following the graph from Figure 1, we wanted to look at the rate of Monkeypox infections per region. In the bar graph above, Western Europe had the greatest number of Monkeypox infections however when we look at the rate of infections, we see that over the course of the 4 months, Southern Europe had the greatest rate of infection with a peak of 2.84 monkeypox cases per 100,000 people in the month of July.
