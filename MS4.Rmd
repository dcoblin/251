...
>>>>>>> 0317ae09ed2847db0e9edbd139d385fe262a3352
```
...
demo_data <- left_join(euro_census_stats2, euro_mpx_cases2, by = "COUNTRY_CODE") %>%
  ...
  pivot_wider(names_from = AGE, values_from = age_cat_pct) %>%
  group_by(COUNTRY_CODE, SEX) %>%
  select(-c(total_pop)) %>%
  arrange(desc(total_cases))

kable(demo_data, "latex", booktabs = T,
      col.names = c("Country",
                    "Sex",
                    "MPX Cases",
                    "Age < 15 (%)",
                    ...
                    "Age 65-84 (%)",
                    "Age 85 and Older (%)"),
      caption = "European Countries by Sex and Age Category") %>%
  kable_styling(latex_options = "HOLD_position")
